...
public class AESEncript {
     ...
     */
    private static char[][] roundKey = new char[AESConstants.BLOCK_LENGTH / 4][AESConstants.BLOCK_LENGTH / 4];

    /**
     * Static state matrix.
     */
    private static char[][] state = new char[AESConstants.BLOCK_LENGTH / 4][AESConstants.BLOCK_LENGTH / 4];


    /**
     * Static expanded key.
     */
    private static char[] expandedKey = new char[AESConstants.EXPANDED_KEY_SIZE];
     ...
     */
    private static byte[] output = new byte[AESConstants.BLOCK_LENGTH];

    /**
     * Static chars array for converting byteToChars.
     */
    private static char[] charsFromBytes = new char[AESConstants.BLOCK_LENGTH];
    ...
    private static void fillInitialMatrix(char[] bytes, char[][] matrix, int from) {
        for (int i = 0; i < AESConstants.BLOCK_LENGTH; ++i) {
            int y = i % 4;
            int x = (i - y) / 4;
    ...
    private static void addRoundKey(char[][] state, char[][] roundKey) {
        for (int i = 0; i < AESConstants.BLOCK_LENGTH / 4; ++i) {
            for (int j = 0; j < AESConstants.BLOCK_LENGTH / 4; ++j) {
    ...
    private static void subBytes(char[][] state) {
        state[0][0] = AESConstants.getSboxValue(state[0][0]);
        state[0][1] = AESConstants.getSboxValue(state[0][1]);
        state[0][2] = AESConstants.getSboxValue(state[0][2]);
        state[0][3] = AESConstants.getSboxValue(state[0][3]);

        state[1][0] = AESConstants.getSboxValue(state[1][0]);
        state[1][1] = AESConstants.getSboxValue(state[1][1]);
        state[1][2] = AESConstants.getSboxValue(state[1][2]);
        state[1][3] = AESConstants.getSboxValue(state[1][3]);

        state[2][0] = AESConstants.getSboxValue(state[2][0]);
        state[2][1] = AESConstants.getSboxValue(state[2][1]);
        state[2][2] = AESConstants.getSboxValue(state[2][2]);
        state[2][3] = AESConstants.getSboxValue(state[2][3]);

        state[3][0] = AESConstants.getSboxValue(state[3][0]);
        state[3][1] = AESConstants.getSboxValue(state[3][1]);
        state[3][2] = AESConstants.getSboxValue(state[3][2]);
        state[3][3] = AESConstants.getSboxValue(state[3][3]);
    }
    ...
    public static char galoisMultiplication(char a, char b) {
        if (a == 0 || b == 0) return 0;
        char s;
        s = (char) ((int) AESConstants.getLTableValue((int) a) + (int) AESConstants.getLTableValue((int) b));
        s %= 255;
        s = AESConstants.getATableValue((int) s);
    ...
    private static void mixColumns(char[][] state) {
        int n = AESConstants.BLOCK_LENGTH / 4;
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
    ...
    private static void applyCore(char[] word, int round) {
        rotate(word);
        for (int i = 0; i < 4; ++i) {
            word[i] = AESConstants.getSboxValue(word[i]);
        }

        word[0] = xor(word[0], AESConstants.getRCONValue((char) round));
    }
    ...
    private static char[] keyExpansion(char[] key) {
        ...
        for (int i = 0; i < AESConstants.KEY_SIZE; ++i) {
            expandedKey[i] = key[i];
        }

        currentSize += AESConstants.KEY_SIZE;

        while (currentSize < AESConstants.EXPANDED_KEY_SIZE) {
            for (int i = 0; i < n; ++i) {
                temp[i] = expandedKey[(currentSize - n) + i];
            }

            if (currentSize % AESConstants.KEY_SIZE == 0) {
                applyCore(temp, rconIteration++);
            }

            for (int i = 0; i < n; ++i) {
                char expandedKeyFormer = expandedKey[currentSize - AESConstants.KEY_SIZE];
    ...
    public static byte[] AES(char[] key) {
        ...
        for (int i = 1; i < AESConstants.ROUNDS; ++i) {
            createRoundKey(expandedKey, i * AESConstants.BLOCK_LENGTH);
            AESRound(state, roundKey);
        }

        createRoundKey(expandedKey, AESConstants.ROUNDS * AESConstants.BLOCK_LENGTH);
        subBytes(state);
        shiftRows(state);
        addRoundKey(state, roundKey);
